...
class HttpCacheManager {
  ...
  _onError(DioError e) async {
    if ((e.request.extra[DIO_CACHE_KEY_TRY_CACHE] ?? false) == true) {
      ...
      if (null != responseDataFromCache)
        return _buildResponse(responseDataFromCache, responseDataFromCache?.statusCode, e.request);
    }
    return e;
  }
  ...
  Response _buildResponse(CacheObj obj, int statusCode, RequestOptions options) {
    Headers headers;
    if (null != obj.headers) {
      headers = Headers.fromMap((Map<String, List<dynamic>>.from(jsonDecode(utf8.decode(obj.headers))))
  ...
  void _tryParseHead(Response response, Duration maxStale, _ParseHeadCallback callback) {
    ...
    if (null != cacheControl) {
      ...
      var parameters;
      try {
        parameters = HeaderValue.parse(cacheControl, parameterSeparator: ",", valueSeparator: "=").parameters;
      } catch (e) {
        print(e);
      }
  ...
  Future<bool> delete(String primaryKey, {String subKey}) => _manager?.delete(primaryKey, subKey: subKey);

  /// no matter what subKey is, delete local cache if primary matched.
  Future<bool> deleteByPrimaryKeyWithUri(Uri uri) => delete(_getPrimaryKeyFromUri(uri));

  Future<bool> deleteByPrimaryKey(String path) => deleteByPrimaryKeyWithUri(_getUriByPath(_baseUrl, path));

  /// delete local cache when both primaryKey and subKey matched.
  Future<bool> deleteByPrimaryKeyAndSubKeyWithUri(Uri uri, {String subKey, dynamic data}) =>
      delete(_getPrimaryKeyFromUri(uri), subKey: subKey ?? _getSubKeyFromUri(uri, data: data));

  Future<bool> deleteByPrimaryKeyAndSubKey(String path,
          {Map<String, dynamic> queryParameters, String subKey, dynamic data}) =>
      deleteByPrimaryKeyAndSubKeyWithUri(_getUriByPath(_baseUrl, path, data: data, queryParameters: queryParameters),
          subKey: subKey, data: data);
